...
params2edr <- function(params, sep = ".", prospect = TRUE, version = 5) {
  ...
  for (pft in distinct_pfts) {
    ...
    if (prospect) {
      stopifnot(version == "5")
      result$spectra_list[[pft]] <- rrtm::prospect5(
        N = pft_traits["prospect_N"],
        Cab = pft_traits["prospect_Cab"],
        Car = pft_traits["prospect_Car"],
        Cbrown = 0,
        Cw = pft_traits["prospect_Cw"],
        Cm = pft_traits["prospect_Cm"]
      )
    }
