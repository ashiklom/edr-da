...
The full canopy radiation profile in ED2 is defined by a vector $\vec{A}$ that contains two hemispherically-integrated radiative fluxes---upward ($\vec{F}_{\up,i}$) and downward ($F_{\down,i}$)---for each cohort $i$,
plus a downward total radiative flux from the atmosphere ($F_{\down,\sky}$) and an upward radiative flux from the ground ($F_{\up,\ground}$) surface (total size $2n + 2$):
...
The change in radiative flux as it encounters canopy elements is governed by the following differential equations (adapting the simplified notation of \citealt{yuan2017reexamination}):

\begin{equation}
  - \frac{dF_{\up, i}}{dx} =
  - \underbrace{(a_{i} + \gamma_{i}) F_{\up, i}}_{\text{Interception}}
  + \underbrace{\gamma_{i} F_{\down, i}}_{\text{Diffuse scatter}}
  + \underbrace{s_{i} F_{0}}_{\text{Direct backscatter}}
\end{equation}

\begin{equation}
  \frac{dF_{\down, i}}{dx} =
  - \underbrace{(a_{i} + \gamma_{i}) F_{\down, i}}_{\text{Interception}}
  + \underbrace{\gamma_{i} F_{\up, i}}_{\text{Diffuse scatter}}
  + \underbrace{s'_{i} F_{0}}_{\text{Direct scatter}}
\end{equation}
...
and $F_{0, i}$ is the incident direct (or ``beam'') radiative flux at canopy layer $i$.
...
\begin{equation}
  s_{i} = \frac{1}{\mu_{0,i}} \omega_{i} \beta_{0,i}
\end{equation}

\begin{equation}
  s'_{i} = \frac{1}{\mu_{0,i}} \omega_{i} \left( 1 - \beta_{0,i} \right)
\end{equation}
...
$\mu_{0}$ is the optical depth per unit plant area index for direct radiation,
and $\beta_{0}$ is the backscattering coefficient for direct radiation.

For a given incident radiation (i.e., solar zenith) angle $\theta$ and leaf orientation angle $\vartheta$, the optical depth is defined as:

\begin{equation}\label{eqn:opticaldepth}
  \mu \left( \theta, \vartheta \right) = \frac{\cos(\theta)}{G(\theta, \vartheta)}
\end{equation}

where $G(\theta, \vartheta)$ is a function describing the projected leaf area.
Following \citet{goudriaan_1977_crop}, this function can be approximated as:

\begin{equation}\label{eqn:gfunction}
  G(\theta, \vartheta) \approx G^*(\theta, \chi_{i}) = \phi_{1, i} + \phi_{2, i} \cos(\theta)
\end{equation}
...
The optical depth for direct radiation, $\mu_{0,i}$, is defined as:

\begin{equation}
  \mu_{0, i} = \mu(\theta, \chi_{i})
\end{equation}
...
Following \citet{sellers1985canopy}, scattering ($\omega$) and backscattering ($\beta$) of canopy elements (leaves or stems) are defined as a function of those elements' reflectance ($R$) and transmittance ($T$; wood transmittance is assumed to be zero).
