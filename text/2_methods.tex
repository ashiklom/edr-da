...
total chlorophyll content (\unit{\mu g ~ cm^{-2}}),
total carotenoid content (\unit{\mu g ~ cm^{-2}}),
water content (\unit{g ~ cm^{-2}}),
and dry matter content (\unit{g ~ cm^{-2}})
...
For soil reflectance, we used the same approach as the ED2 default (weighted average of wet and dry soil reflectance), but with full 1 \unit{nm} soil spectra from Hapke (REF). % TODO: Flesh this out
...
For model calibration, we selected 47 sites from the NASA Forest Functional Types (FFT) field campaign that contained plot-level inventory data (stem density, species identity, and DBH) coincident with observations of the NASA Airborne Visible/Infrared Imaging Spectrometer (AVIRIS).
...
We calibrated EDR using the same general Bayesian inversion as in Chapter 3.
...
Because of unrealistic values in the shortwave infrared spectral region in the AVIRIS observations, likely caused by faulty atmospheric correction, I only calibrated the model with observations from 400 to 1300 \unit{nm}.
...
For priors on the leaf biomass allometry parameters, I fit a multivariate normal distribution to allometry coefficients from \citet{jenkins_2003_allom,jenkins_2004_allom} using the \texttt{PEcAn.allometry} package.
