...
%.css: 
	...
	Rscript testruns/generate_testrun_single_cohort_USWCr.R $(dt) $(pt) $(st)
...
clean:
	rm -rf ed-inputs/sites/US-WCr/rtm/1cohort \
	    run-ed/1cohort run-ed/template/ed_2.1 run-ed/template/ed_2.1-opt run-ed/template/ED2IN ed-inputs/met3/US-WCr/ED_MET_DRIVER_HEADER
